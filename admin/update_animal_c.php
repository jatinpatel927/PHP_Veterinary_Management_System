...
if(isset($_POST['submit']))
{
    ...
    if($animal == !"")
     {
        $animal_query = "UPDATE `animal_category` SET `a_id` = '$a_id',`c_animal_name` = '$animal' WHERE `animal_category`.`id` = '$id'";
        $animal_run = mysqli_query($con,$animal_query);
        if($animal_run)
        {
            echo "<script>alert('One Animal Category Updated')</script>";
            ?>
            <script>
        ...
        {
            echo "<script>alert('Animal Category not Updated')</script>";
            ?>
            <script>
                window.location.href='update_animal_c.php';
            </script>
            <?php
        }
    }
