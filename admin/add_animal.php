<?php require 'database/connect.php';
   if(isset($_POST['submit']))
   {
    // echo "hell";
     ...
     if($animal == !"")
     {
        $animal_query = "INSERT INTO `animal` (`animal_name`, `date`) VALUES ('$animal', current_timestamp())";
        $animal_run = mysqli_query($con,$animal_query);
        if($animal_run)
        {
            echo "<script>alert('One Animal Add')</script>";
            ?>
            <script>
        ...
        {
            echo "<script>alert('Animal not Add')</script>";
            ?>
            <script>
